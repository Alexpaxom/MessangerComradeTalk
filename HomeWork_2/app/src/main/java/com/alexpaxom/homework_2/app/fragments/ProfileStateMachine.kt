...
sealed interface ProfileState {
    ...
    class ResultState(val user: UserItem): ProfileState {
        override fun toResultState(): Boolean = true
        override fun toLoadingState(): Boolean = true
    ...
    class ErrorState(val error: Throwable): ProfileState {
        override fun toLoadingState(): Boolean = true
    }
...
interface ProfileStateMachine {
    ...
    fun goToState(newState: ProfileState) {
        val canGoToState = when(newState) {
            ...
            is ProfileState.InitialState -> { currentState.toInitialState() }
        }
    ...
    fun toResult(resultState: ProfileState.ResultState){}
    fun toLoading(loadingState: ProfileState.LoadingState){}
    fun toError(errorState: ProfileState.ErrorState){}

    fun defaultToStateCallback(nextState: ProfileState): Boolean {
